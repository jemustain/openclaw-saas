...
function Inner() {
  ...
  useEffect(() => {
    ...
    if (callbackError) {
      ...
      setError(messages[callbackError] ?? 'Something went wrong. Please try again.');
    }
  }, [searchParams]);
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gray-950 px-4">
      <div className="w-full max-w-sm space-y-6">
        ...
        <button onClick={handleGoogleLogin} disabled={googleLoading} className="w-full flex items-center justify-center rounded-lg border border-gray-700 bg-gray-900 px-4 py-2.5 text-sm text-white hover:bg-gray-800 disabled:opacity-50 transition">
          <GoogleIcon />
