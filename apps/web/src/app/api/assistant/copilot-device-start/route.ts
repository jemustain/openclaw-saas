...
/**
 * POST /api/assistant/copilot-device-start
 * Start GitHub Copilot device flow on the user's VM.
 */
export async function POST() {
  try {
    ...
    const sidecarRes = await fetch(
      `http://${assistant.ip_address}:${SIDECAR_PORT}/openclaw/github-copilot-device-start`,
      {
        method: 'POST',
        headers: {
    ...
    if (!sidecarRes.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to start device flow on VM', details: err },
        { status: 502 },
      );
    }
    ...
    return handleApiError(err, 'assistant/copilot-device-start');
  }
}
